// Country dial codes for the phone input. Each entry has the ISO 3166-1
// alpha-2 country code (used to derive the flag emoji at runtime), the
// English country name (for searching), and the international dial code
// without the leading "+". Sorted alphabetically by name with a small
// "popular" set surfaced first by `phone-input.tsx`.
...
export const POPULAR_ISO = ["US", "CA", "GB", "AU", "IN", "MX", "BR", "DE", "FR", "ES"] as const;
...
export const countries: Country[] = [
  ...
  { name: "Turks and Caicos", iso: "TC", dial: "1649" },
 ...
 * Unicode regional-indicator letters. Falls back to a globe glyph for
 * codes that don't map cleanly (e.g. "XK" Kosovo on older platforms).
 */
export function flagFromIso(iso: string): string {
  if (iso.length !== 2) return "\uD83C\uDF10";
  const base = 0x1f1e6;
  const a = "A".charCodeAt(0);
  const first = iso.toUpperCase().charCodeAt(0) - a + base;
  const second = iso.toUpperCase().charCodeAt(1) - a + base;
  return String.fromCodePoint(first, second);
}

/**
 * Resolve a stored E.164-ish phone string ("+15551234567") to the
 * country whose dial code prefixes it. Longer dial codes win (so "1242"
 * resolves to Bahamas before falling through to "1" for the US/CA).
 */
export function detectCountry(value: string, fallbackIso = "US"): Country {
  const digits = value.replace(/[^\d]/g, "");
  if (!digits) return countries.find((c) => c.iso === fallbackIso) ?? countries[0]!;
  const sorted = [...countries].sort((a, b) => b.dial.length - a.dial.length);
  const match = sorted.find((c) => digits.startsWith(c.dial));
  if (match) return match;
  return countries.find((c) => c.iso === fallbackIso) ?? countries[0]!;
}

export function stripDialPrefix(value: string, dial: string): string {
  const digits = value.replace(/[^\d]/g, "");
