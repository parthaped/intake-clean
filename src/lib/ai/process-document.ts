...
import type { DocumentClassificationResult, OCRResult, QualityCheckResult } from "@/lib/ai/types";
...
async function persistAndReturn(args: PersistArgs): Promise<ProcessDocumentResult> {
  const service = getServiceSupabase();
  const { file, organizationId, classification, ocrResult, quality, status, providerName, hfModelUsed, hfLatencyMs } =
    args;
